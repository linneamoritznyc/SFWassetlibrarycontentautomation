...
**Done on 21 September, nothing left to fetch.** The font is now in the
repository (`packages/brand/fonts/Montserrat-SemiBold.ttf`, Open Font License)
and both worker images copy it in rather than downloading it. The download URL
had in fact stopped working, and the caption style was asking for a name libass
does not match, so every clip would have come out in DejaVu. A caption was
burned with ffmpeg during the fix and libass reported picking
`Montserrat-SemiBold`. See `docs/DECISIONS.md`, "After Phase 8".

**What is still worth a glance:** after the first deploy, cut one clip and
look at it. If the caption is not Montserrat, the Railway build log for
`worker-media` will say why at the `COPY packages/brand/fonts` step.
